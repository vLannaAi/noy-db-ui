...
import { z } from 'zod'
import { createNoydb, dictKey, ref, type FieldMeta } from '@noy-db/hub'
...
async function setup() {
  ...
  const tasks = vault.collection('tasks', {
    ...
    dictKeyFields: { status: dictKey('taskStatus', TASK_STATUS) },
    fieldMeta: taskFieldMeta,
  })
  await users.put('u1', { id: 'u1', name: 'Ada', team: 'core' })
  await users.put('u2', { id: 'u2', name: 'Lin', team: 'web' })
  await tasks.put('t1', { id: 't1', title: 'A', ownerId: 'u1', ownerName: 'Ada', status: 'todo', estimate: 3 })
  await tasks.put('t2', { id: 't2', title: 'B', ownerId: 'u2', ownerName: 'Lin', status: 'done', estimate: 5 })
  return { tasks, users }
}
