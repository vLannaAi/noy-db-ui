...
const tok = (s: string) => (needsQuote(s) ? `"${s}"` : s)

function valueSuggestionsFor(field: FieldDef, partial: string, schema: EntitySchema, facets: Record<string, string[]>): Suggestion[] {
  const p = normalizeTerm(partial)
  const out: Suggestion[] = []
  if (field.type === 'enum') {
    const values = field.enumOrder ? [...field.enumOrder] : (facets[field.id] ?? [])
    for (const v of values) if (!p || normalizeTerm(v).includes(p)) out.push({ label: `${field.label}: ${v}`, hint: 'filter', token: `${field.id}:${tok(v)}` })
  } else {
    for (const v of facets[field.id] ?? []) if (!p || normalizeTerm(v).includes(p)) out.push({ label: `${field.label}: ${v}`, hint: 'filter', token: `${field.id}:${tok(v)}` })
  }
  return out
}
...
function sortSuggestions(schema: EntitySchema, partial: string): Suggestion[] {
  ...
  for (const f of schema.fields) {
    ...
    const up: Suggestion = { label: `Sort: ${f.label} ↑`, hint: 'a→z', token: `sort:${f.id}-up` }
    const down: Suggestion = { label: `Sort: ${f.label} ↓`, hint: 'z→a', token: `sort:${f.id}-down` }
...
function groupSuggestions(schema: EntitySchema, partial: string): Suggestion[] {
  const p = normalizeTerm(partial)
  const out: Suggestion[] = []
  for (const f of schema.fields) {
    ...
    const names = [f.id, f.label, ...(f.aliases ?? [])].map(normalizeTerm)
    if (p && !names.some((x) => x.includes(p))) continue
    out.push({ label: `Group by ${f.label}`, hint: 'group', token: `group:${f.id}` })
  }
  return out
}
...
function viewSuggestions(schema: EntitySchema, columns: ColumnRef[], partial: string, kind: 'show' | 'hide'): Suggestion[] {
  const p = normalizeTerm(partial)
  ...
  for (const s of src) {
    ...
    out.push({ label: `${kind === 'show' ? 'Show' : 'Hide'} ${s.label}`, hint: kind, token: `${kind}:${s.id}` })
  }
  return out
}
...
export function buildSuggestions(
  ...
  if (colon > 0) {
    ...
    if (cmd === 'sort' || cmd === 'sort-by' || cmd === 'sortby') {
      return sortSuggestions(schema, partial).slice(0, MAX)
    }
    // group: / group-by: → groupable field list
    if (cmd === 'group' || cmd === 'group-by' || cmd === 'groupby') {
      return groupSuggestions(schema, partial).slice(0, MAX)
    }
    // show: / hide: → force a column visible/hidden (R5)
    if (cmd === 'show' || cmd === 'hide') {
      return viewSuggestions(schema, columns, partial, cmd).slice(0, MAX)
    }
    ...
    if (field) {
      if (field.type === 'date') {
        ...
        out.push(...valueSuggestionsFor(field, partial, schema, facets))
      }
  ...
  // field name matches → `field:` (two-step)
  for (const f of schema.fields) {
    const names = [f.id, f.label, ...(f.aliases ?? [])].map(normalizeTerm)
    if (names.some((x) => x.includes(n))) out.push({ label: `${f.label}:`, hint: 'field', token: `${f.id}:`, caretAfter: 'value' })
  }

  // enum value matches (incl. synonyms) → field:value
  for (const f of schema.fields) {
    ...
    if (candidates.has(canon)) out.push({ label: `${f.label}: ${canon}`, hint: 'filter', token: `${f.id}:${tok(canon)}` })
    else for (const v of f.enumOrder) if (normalizeTerm(v).includes(n)) out.push({ label: `${f.label}: ${v}`, hint: 'filter', token: `${f.id}:${tok(v)}` })
  }
  ...
  out.push({ label: `Search “${d}”`, hint: 'text', token: tok(d) })
