          ...
          >
            <div class="inline-flex items-center gap-1.5" :class="aggAlign(col) === 'text-right' ? 'flex-row-reverse' : ''">
              ...
              <span v-if="subtotalEnums?.[col.key]" class="text-xs text-nui-muted leading-snug">
                <template v-for="(it, i) in subtotalEnums[col.key]!.items" :key="it.value"><span v-if="i" class="opacity-50"> · </span><span class="whitespace-nowrap"><span class="font-semibold text-nui-fg">{{ it.count }}</span> {{ it.value.replace(/_/g, ' ') }}</span></template>
              </span>
              <span
                v-else-if="agg(col)"
